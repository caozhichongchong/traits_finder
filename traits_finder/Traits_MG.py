...
parser.add_argument("--fa",
                    help="input format of metagenomes sequence",
                    type=str, default='.fasta',metavar='.fasta, .fna or .fa')
...
parser.add_argument('--bp',
                    help="Optional: complete path to blastx or blastn if not in PATH, \'None\' for no blast search",
...
# makedb for 16S
try:
    ftest = open(workingdir +'/database/85_otus.fasta.all.V4_V5.fasta.udb', 'r')
except IOError:
    if args.u != 'None':
        if 'usearch' in args.u:
            f1.write(os.path.join(os.path.split(args.u)[0],
...
try:
    ftest = open(workingdir +'/database/85_otus.fasta.all.V4_V5.fasta.counts', 'r')
except IOError:
    if args.u != 'None':
...
for file_name in list_of_files:
    if not any(files in file_name for files in ['all.sh','SearchMG.sh']):
        f1.write("sbatch -p sched_mem1TB -c 40 -t 5-00:00:00 --mem=500000 -J " + os.path.split(str(file_name))[-1] + "traits -o " + str(file_name) + ".out -e " + str(file_name) + ".err " + str(file_name) +" \n")
