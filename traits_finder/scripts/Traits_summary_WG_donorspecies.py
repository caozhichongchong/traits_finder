...
parser.add_argument("-db",
                    help="file name of your input database nucleotide sequences of genes",
                     type=str, default='Butyrate.all.spades.fna',metavar='.all.spades.fna')
...
result_dir = os.path.split(args.r)[0]
try:
    os.mkdir(os.path.join(result_dir,'../../summary'))
except OSError:
    pass
...
def freq_call_sub(vcf_file,all_wrong_gene,vcf_calculate=False):
        SNP = dict()
        SNP_count = dict()
        ...
        for lines in open(vcf_file, 'r'):
            if not lines.startswith("#"):
                ...
                if Depth >= 1:
                    ...
                    Chr = contig_to_gene(Chr, Position)
                    if Chr != '':
        ...
        if vcf_calculate:
            ...
            return [SNP,SNP_count, Total, SNP_count_genome_count,SNP_type_sum]
        else:
...
def freq_call(vcf_file,cov_file,Not_pass=dict()):
    ...
    try:
        SNP, SNP_count, Total,SNP_count_genome_count,SNP_type_sum = freq_call_sub(vcf_file,all_wrong_gene,True)
        SNP_all, SNP_count_all, Total = freq_call_sub(cov_file,all_wrong_gene,False)
        ...
        for Chr_position in SNP_all:
            ...
            if Chr not in all_wrong_gene:
                ...
                SNP_gene_sum.addposition(Chr, position, Depth_position)
                ...
                if Chr_position in SNP:
                    ...
                    SNP_gene_sum.addalt(AllALT_frq)
                    if refSNP_pair_sum_all != 'None':
                        ...
                        if codon_start <= position - 1:
                            ...
                            if len(Ref_seq_codon) == 3:
                                ...
                                for minor in Minor_ALT:
                                    ...
                                    SNP_gene_sum.addSNP_pair(SNP_pair, N_S_set[temp_NorS],
                                                             ALT_frq, 1, Depth_position)
                                    SNP_gene_sum.addmutposition(Chr, position)
                    else:
                        for minor in Minor_ALT:
                            ALT = minor[0]
                            ALT_frq = minor[1]
                            lines += '\t%s\t%s:%s:None\t%s:%s:None\tNone\t%s' % ('%s' % (Chr_position_genome_count[0]),
                                                                                 REF, Ref_frq,
                                                                                 ALT, ALT_frq,
        ...
        if all_SNP_gene_temp!= dict():
            ...
            for Chr in all_SNP_gene_temp:
                ...
                filter_result = filtersnp(SNP_gene_temp, SNP_gene_sum, Chr, all_output_filter_list, sample_name)
            ...
            filtersnp(SNP_gene_sum, SNP_gene_sum, 'all', all_output_filter_list, sample_name)
            ...
            foutput.write('#CHR\tPOS\tDEP\tA\tT\tG\tC\tALT_frq\tgenome_REF_ALT_count\tREF:count:REF_aa\tALT:count:ALT_aa\tN_or_S\tgenotype_allgenomes\n')
...
def SNP_dynamics(output_files, SNP_outputfile):
    ...
    for files in output_files:
        metadata = os.path.split(files)[1].split('.flt.snp.vcf.filtered.vcf')[0]
...
try:
    for lines in open(args.db + '.length', 'r'):
        Mapping.setdefault(lines.split('\t')[0],
                               float(lines.split('\t')[1]))
    for lines in open(args.db + '.loci', 'r'):
        Mapping_loci.setdefault(lines.split('\t')[0],
                               [float(lines.split('\t')[1]),
                                float(lines.split('\t')[2]),
                                lines.split('\t')[3]])
except (IOError,FileNotFoundError):
    ...
    for record in SeqIO.parse(Fasta_name, 'fasta'):
        output_length.append(str(record.id) + '\t' + str(
            len(record.seq)) + '\t\n')
        Mapping.setdefault(str(record.id), len(str(record.seq)))
        description = str(record.description).replace(' ','').split('#')
        contig = '_'.join(str(record.id).split('_')[0:-1])
        output_loci.append(contig + '\t' + description[1]
                           + '\t' + description[2]
                            + '\t' + str(record.id) + '\t\n')
    ...
    f.write(''.join(output_length))
    f.close()
    f = open(args.db + '.loci', 'w')
    f.write(''.join(output_loci))
    f.close()
...
if args.s == 3:
    ...
    # sum up coverage, SNP and dN dS
    try:
        all_output = open((os.path.join(result_dir, '../../summary/%s.all.bam.cov.sum.snp.NS.ratio')%(reference_database)), 'r')
    except (IOError,FileNotFoundError):
        SNP_gene_sum = SNP_gene()
        SNP_gene_sum.init('allspecies')
        all_output = open((os.path.join(result_dir, '../../summary/%s.all.bam.cov.sum.snp.NS.ratio')%(reference_database)), 'w')
        all_output2 = open((os.path.join(result_dir, '../../summary/%s.all.bam.cov.sum.snp.NS.ratio.snp.only') % (reference_database)), 'w')
        all_output_filter = open((os.path.join(result_dir, '../../summary/%s.all.bam.cov.sum.nopass.gene')%(reference_database)), 'w')
        all_output.write(
                'genome\tgene\tnumber_sample\tdepth\tcoverage\tgene_length\ttotal_SNP\ttotal_SNP_position\tavg_minor_freq\tsequence_type_count\tSNP_dis_0\tSNP_dis_1\tSNP_dis_2\tSNP_codon_0\tSNP_codon_1\tSNP_codon_2\tN:S\tobserved_ratio\texpected_ratio_norm\tdNdS_norm\texpected_ratio\tdNdS\tA-T_freq\tA-T_N:S' +
            '\tA-C_freq\tA-C_N:S\tG-C_freq\tG-C_N:S\tG-T_freq\tG-T_N:S\tA-G_freq\tA-G_N:S\tG-A_freq\tG-A_N:S\n')
        all_output2.write(
                'genome\tgene\tnumber_sample\tdepth\tcoverage\tgene_length\ttotal_SNP\ttotal_SNP_position\tavg_minor_freq\tsequence_type_count\tSNP_dis_0\tSNP_dis_1\tSNP_dis_2\tSNP_codon_0\tSNP_codon_1\tSNP_codon_2\tN:S\tobserved_ratio\texpected_ratio_norm\tdNdS_norm\texpected_ratio\tdNdS\tA-T_freq\tA-T_N:S' +
            '\tA-C_freq\tA-C_N:S\tG-C_freq\tG-C_N:S\tG-T_freq\tG-T_N:S\tA-G_freq\tA-G_N:S\tG-A_freq\tG-A_N:S\n')
        # calculate allele frequency
        output_files = glob.glob(args.r)
        i = 0
        print('%s summarizing allele frequency and SNPs for %s' % (datetime.now(), os.path.join(args.r)))
        ref_notpass = dict()
        for vcf_file in output_files:
            cov_file = vcf_file.replace('.flt.snp.vcf.filtered.vcf', '.raw.vcf')
            i += 1
            if i%100 == 0:
                print('%s summarizing allele frequency and SNPs for %s files' % (datetime.now(), i))
            freq_call(vcf_file, cov_file, ref_notpass)
        new_line = sum_snp(SNP_gene_sum, SNP_gene_sum, 'all', sample_name, Total, SNP_type_sum.get('all', ''))
        all_output.write(''.join(new_line))
        if SNP_gene_sum.mutpositionsum > 0:
            all_output2.write(''.join(new_line))
        all_output.close()
        all_output2.close()
